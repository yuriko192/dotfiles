...
Ask for approval before starting. If `ask_user` is available, use it.
...
- For each approved slice, create a branch from the right base, stage and commit only the planned files or hunks, then push and open the PR with `gh`.
